...
A((("sets", "set literals"))) sintaxe de literais `set`—`{1}`, `{1, 2}`, etc.—parece exatamente igual à notação matemática, mas tem uma importante exceção: não existe uma notação literal para o `set` vazio, então precisamos nos lembrar de escrever `set()`.
